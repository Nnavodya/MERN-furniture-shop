...
import { TbUser, TbMail, TbLock, TbEye, TbEyeOff, TbArrowRight } from 'react-icons/tb'
      ...
      <div className="container mx-auto px-4 py-16 flex justify-center">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold mb-2" style={{ color: C.text }}>Create your account</h1>
            <p className="text-sm" style={{ color: C.textMuted }}>
              Join FurniHub for faster checkout and order tracking
            </p>
          </div>
          ...
          <div className="rounded-2xl p-8" style={{ background: C.card, border: `1px solid ${C.divider}` }}>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold" style={{ color: C.text }}>Full Name</label>
                <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg" style={{ border: `1px solid ${C.accentBorder}` }}>
                  <TbUser className="h-4 w-4 shrink-0" style={{ color: C.accent }} />
                  <input
                    type="text"
                    placeholder="Nethmi Rajapaksha"
                    value={form.name}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold" style={{ color: C.text }}>Confirm Password</label>
                <div
                  className="flex items-center gap-2 px-3 py-2.5 rounded-lg"
                  style={{ border: `1px solid ${form.confirm && !passwordsMatch ? '#E53935' : C.accentBorder}` }}
                >
                  <TbLock className="h-4 w-4 shrink-0" style={{ color: C.accent }} />
                  <input
                    type={showPw ? 'text' : 'password'}
                ...
                {form.confirm && !passwordsMatch && (
                  <p className="text-xs" style={{ color: '#E53935' }}>Passwords don't match</p>
                )}
              </div>
              ...
              >
                {submitting ? 'Creating account…' : 'Create Account'} <TbArrowRight className="h-4 w-4" />
              </button>
            </form>
          </div>
          ...
          <p className="text-center text-sm mt-6" style={{ color: C.textMuted }}>
            ...
            <Link to="/login" className="font-semibold" style={{ color: C.accent }}>
              Log in
            </Link>
          </p>
        </div>
